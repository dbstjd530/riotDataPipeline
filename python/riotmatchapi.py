...
class RiotMatchData:
    ...
    def allTierUsersId(self, queueType, page):
        challengers = pd.DataFrame(self.challengersleagues_by_queue(queueType)['entries'])['summonerName'].tolist()[:10]
        grandmasters = pd.DataFrame(self.grandmasterleagues_by_queue(queueType)['entries'])['summonerName'].tolist()[:10]
        masters = pd.DataFrame(self.masterleagues_by_queue(queueType)['entries'])['summonerName'].tolist()[:10]
        diamonds = pd.DataFrame(self.otherleagues_by_queue('II', 'DIAMOND', queueType, page))['summonerName'].tolist()[:10] 
        platinums = pd.DataFrame(self.otherleagues_by_queue('II', 'PLATINUM', queueType, page))['summonerName'].tolist()[:10]
        #golds = pd.DataFrame(self.otherleagues_by_queue('II', 'GOLD', queueType, page))['summonerName'].tolist()
                
        allUser = challengers + grandmasters + masters + diamonds + platinums# + golds
        return allUser
